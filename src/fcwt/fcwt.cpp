...
void saveFFTWComplexArraysForPlot(fftwf_complex *array, int size, const std::string &filename) {
    ...
    for (int i = 0; i < size; ++i) {
        file << i << " " << array[i][0] << " " << array[i][1] << std::endl;
    }
...
void saveFFTRealPartForPlot(fftwf_complex *data, int size, const char *filename) {
    ...
    for (int i = 0; i < size; i++) {
        fprintf(file, "%d %f\n", i, data[i][0]);  // Сохраняем только реальную часть
    }
    fclose(file);
}
...
// Initialize intermediate result
    ...
    saveFFTWComplexArraysForPlot(Ihat, newsize, "Ihat.fft");
